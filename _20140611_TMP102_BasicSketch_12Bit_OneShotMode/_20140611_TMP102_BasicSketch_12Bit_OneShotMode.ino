...
{ 
  ...
  if ( errorflag != 0) {
    Serial.print ("Initial control reg writing on TMP102 failed!");
    errorflag=0;
  }
  ...
  errorflag = Wire.endTransmission();
  if ( errorflag != 0) {
    Serial.print ("OOPS! Problem setting OneSHOT bit! ");
    errorflag=0;
  } 

  getTMP102();  
  Serial.println("Success:TMP102 has been initialized");
...
void getTMP102() {
  ...
  errorflag = Wire.endTransmission();
  if ( errorflag != 0) {
    Serial.print ("TMP102 clearing OS bit in CFG reg failed...");
    errorflag=0;
  } 
  ...
  errorflag = Wire.endTransmission();
  if ( errorflag != 0) {
    Serial.print ("Can't set temp reg pointer...");
    errorflag=0;
  }
  ...
  Serial.print("Integer data before conversion: ");
  Serial.println(Temp16);
  deg_c = Temp16*0.0625;
  Serial.print("Temperature in deg C = ");
  Serial.println(deg_c,4);
